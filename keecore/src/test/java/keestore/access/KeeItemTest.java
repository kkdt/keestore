...
public class KeeItemTest {
   ...
   public void testNewKeeItem() {
      ...
      assertTrue("Expected a new KeeItem with one key-value pair (the name field)", 
         item != null && item.size() == 1 && name.equals(item.getName()));
   }
   ...
   public void testToMap() {
      ...
      assertTrue(itemCopy.size() == item.size() 
         && itemCopy.toMap().equals(item.toMap()));
   }
   ...
   public void testToJson() {
      ...
      assertTrue("Converted and original items do not have the same keys", convertedItem.keys().containsAll(item.keys()));
