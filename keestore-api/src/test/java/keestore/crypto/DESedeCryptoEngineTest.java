...
public class DESedeCryptoEngineTest extends KeyCryptoUnitTest {
    ...
    byte[] loadSecretKey() throws Exception {
        ...
        SecretKey loadedKey = keyfactory.generateSecret(
                new SecretKeySpec(crypto.generateKey(password), 0, cryptoContext.getSecretKey().length, algorithm));
    ...
    public void testReloadSecretKey() throws Exception {
        ...
        assertTrue("The initially secret key must not equal the re-loaded key",
                !Arrays.equals(anotherSecret, cryptoContext.getSecretKey()));
    }
